...
export default function Systematic9() {
  const { activeGames } = useGameServerData();

  const [values, setValues] = useState([[-1, -1, -1, -1, -1, -1, -1, -1, -1]]);

  const [activeButton, setActiveButton] = useState([0]);
  const [activeInput, setActiveInput] = useState(0);
  const [lotteries, setLotteries] = useState(1);
  const [tables, setTables] = useState(1);
  const [pay, setPay] = useState(269.9);
  ...
  useEffect(() => {
    setPay(Math.round((269.9 * lotteries + Number.EPSILON) * 100) / 100);

    const currGameIdx = activeGames.findIndex(
      (game) => game.gameType === "777" && game.gameName === "Systematic 9",
    );
          ...
          <div className="flex flex-col gap-2">
            ...
            <ul className="flex w-fit flex-col gap-2">
              ...
              <li className="flex items-center gap-2 text-sm text-gray-600">
                ...
                <span className="w-56">Guess 9 numbers out of 70</span>
              </li>
              <li className="flex items-center gap-2 text-sm text-gray-600">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
