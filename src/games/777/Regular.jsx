...
function Regular() {
  ...
  const [activeButton, setActiveButton] = useState([0, 0, 0]);
  ...
  useEffect(() => {
    let totalTables = 0;
    for (let i = 0; i < 3; i++) {
      if (values[i].some((val) => val !== -1)) {
        totalTables++;
      }
    }

    const currGameIdx = activeGames.findIndex(
      (game) => game.gameType === "777" && game.gameName === "Regular",
    );
    const currGameId = activeGames[currGameIdx]?._id;
    setGameId(currGameId);

    setPay(
      Math.round((totalTables * 9.9 * lotteries + Number.EPSILON) * 100) / 100,
    );
    setTables(totalTables);
  }, [values, lotteries, activeGames]);

  function fillRandomVals(idx = -1) {
    const fillVals = (i) => {
      const valCopy = [...values[i]];
      valCopy.forEach((i, idx) => {
        if (i !== -1) return;
        valCopy[idx] = Math.floor(Math.random() * 70);
      });

      const valueDuplicate = [...values];
      valueDuplicate[i] = valCopy;
    ...
    if (idx === -1) {
      ...
      fillVals(idx);
      setActiveInput((activeInput + 1) % 3);
    }
  }
          ...
          <div className="flex h-fit w-fit flex-col gap-2 rounded-2xl bg-gray-300 p-2">
            ...
            {values.map((item, idx) => (
              ...
              >
                  ...
                  <svg
                    ...
                    className={`${
                      values[idx].every((i) => i !== -1)
                        ? "opacity-100"
                        : "opacity-0"
                    } icoNamen icon-tabler icon-tabler-circle-check-filled`}
                    width="20"
                    height="20"
                    viewBox="0 0 24 24"
                    strokeWidth="2"
                ...
                <span className="flex h-5 w-5 items-center justify-center rounded-full border border-gray-400 bg-white text-xs font-bold text-black">
                  {idx + 1}
                </span>
              </button>
            ))}
