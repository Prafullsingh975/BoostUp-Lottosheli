...
export default function GreatChanceGame() {
  const [state, dispatch] = useReducer(reducer, initialState);
  const [pay, setPay] = useState(0);

  useEffect(() => {
    let total = state.lotteries * state.participation;
    total = (total * 29.7) / 100;
    setPay(total);
  }, [state]);
